...
widgetRegister = largePane $ do
    ...
    (_form, _result) <- form $ do -- checkValidity() -> add was-validated class for JS classes
        _dynUsername <- formGroup . divClass "my-2" $ inputBox def {
          inputBoxId = "username",
          inputBoxLabel = "Username *",
          inputBoxPlaceholder = "bob"
        } -- TODO add is-invalid from server
            -- here in small red: optional error, cover box with red border
        _dynEmail <- formGroup $ do
            input <- divClass "my-3" $ inputBox def {
                inputBoxType = "email",
                inputBoxId = "email",
                inputBoxLabel = "Email Address *",
                inputBoxPlaceholder = "bob@bob.com"
            }
        ...
        _dynFullName <- formGroup $ do
            input <- divClass "my-3" $ inputBox def {
                inputBoxId = "fullName",
                inputBoxLabel = "Full Name *",
                inputBoxPlaceholder = "Bob Frog"
            }
            divClass "invalid-feedback" $ text "You must provide this field."
            pure input
        _dynChosenName <- formGroup $ do
            input <- divClass "my-3" $ inputBox def {
                inputBoxId = "chosenName",
                inputBoxLabel = "What should we should call you?",
                inputBoxPlaceholder = "Bob"
            }
        ...
        _dynPassword <- formGroup $ do
            input <- divClass "my-3" $ inputBox def {
                ...
                inputBoxPlaceholder = "A_R3ally_s3cure_pa$$word!"
            }
        ...
        _dynPassword2 <- formGroup $ do
            input <- divClass "my-3" $ inputBox def {
                inputBoxId = "password2",
                inputBoxType = "password",
                inputBoxLabel = "Password (again) *",
                inputBoxPlaceholder = "A_R3ally_s3cure_pa$$word!"
            }
